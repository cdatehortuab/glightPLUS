...
class orm
{
	...
	{	
		for ($i=0,$n=count($class); $i<$n; $i++)		
		{
			...
			$this->data["$class[$i]"] = $this->db->select($options["$class[$i]"],isset($cod["$class[$i]"]) ? $cod["$class[$i]"] : $cod);
	...
	{
		$data = isset($this->data[$class][$pos]) ? $this->data[$class][$pos] : NULL;
		return $data;
	}
